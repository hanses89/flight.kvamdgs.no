...
function calcPath(speed, glide, turn, fade, W, H, hand, throwSpeed, forehand, release) {
  ...
  const dist       = (H - PAD_T - PAD_B) * (distMeters / 160);

  // Net stability index
  const stability = fadeV + turnV;

  // Base turn and fade amounts — calibrated via coordinate descent regression
  // against 109 reference discs across 14 brands (Innova, Discraft, Discmania,
  // Axiom/MVP/Streamline, Kastaplast, Latitude 64, Dynamic Discs, Westside,
  // Prodigy, Gateway, Millennium, Infinite Discs, Legacy, Mint Discs)
  //
  // TURN_K=0.44, FADE_K=0.745, speedFadeMult=1-speedN*0.275, overcastFadeMult=1-ocf*0.835
  // MAE: Turn 0.068 col, Fade 0.296 col across 109 discs
  //
  // Key targets (RHBH flat hard, normal arm):
  //   Aviar    (2/3/0/1):  0 col turn, -0.25 col land
  //   Teebird  (7/5/0/2):  0 col turn, -0.60 col land
  //   Firebird (9/3/0/4):  0 col turn, -1.30 col land
  //   Leopard  (6/5/-2/1): 0.90 col turn, -0.10 land
  //   Roadrunner(9/5/-4/1):2.00 col turn, +0.20 land
  //   Destroyer(12/5/-1/3):0.45 col turn, -0.80 land
  //   Boss     (13/5/-1/3):0.40 col turn, -0.85 land
  //   Colossus (14/6/-4/0):1.80 col turn, +1.00 land

  const baseTurnAmt    = -turnV * COL * 0.44 * turnExpr;
  const speedFadeMult  = Math.max(0.05, 1.0 - speedN * 0.275);
  const powerFadeMult  = throwSpeed === 'slow' ? 0.50 : throwSpeed === 'medium' ? 0.72 : 1.0;
  const overcastFactor = Math.max(0, Math.min(1, powerN - speedN * 0.85));
  const overcastFadeMult = 1.0 - overcastFactor * 0.835;
  const baseFadeAmt    = fadeV * COL * 0.745 * speedFadeMult * powerFadeMult * overcastFadeMult;

  // Timing — calibrated from Boss/Leopard/Firebird DGPuttheads charts:
  // Higher glide = stays in high-speed phase longer → later bend AND later fade
  // Low glide (Firebird/Rhyno) = fades earlier
  const glideTimingBonus = (glideN - 0.5) * 0.08;  // glide 5/7 ≈ +0.04, glide 1/7 ≈ -0.03
  const bendStart = Math.max(0.20,
    (throwSpeed === 'slow' ? 0.42 : throwSpeed === 'medium' ? 0.48 : 0.55)
    - (1 - speedN) * 0.10
    + glideTimingBonus);
  const fadeStart = Math.max(0.60,
    (throwSpeed === 'slow' ? 0.72 : throwSpeed === 'medium' ? 0.80 : 0.88)
    - (1 - speedN) * 0.15
    + glideTimingBonus);
  ...
  // FOREHAND
  if (isForehand) {
    const fhFadeMult  = rel === 'hyzer' ? 2.2 : rel === 'anhyzer' ? 0.4 : 1.0;
    const fhRelOffset = rel === 'anhyzer' ? -COL * 0.30 : rel === 'hyzer' ? COL * 0.25 : 0;
    return {
      p0: { x: cx,                                                       y: yBot },
      p1: { x: cx + mirror * fhRelOffset,                                y: yBot - dist * bendStart },
      p2: { x: cx + mirror * (-baseTurnAmt * 0.5 + fhRelOffset),         y: yBot - dist * fadeStart },
      p3: { x: cx + mirror * (-baseTurnAmt + baseFadeAmt * fhFadeMult),  y: yBot - dist },
      distMeters,
    };
  }
  ...
  if (rel === 'hyzer') {
    if (isHyzerFlip) {
      return {
        p0: { x: cx,                                                          y: yBot },
        p1: { x: cx - mirror * COL * 0.15 * (1 - flipStrength),              y: yBot - dist * bendStart },
        p2: { x: cx + mirror * baseTurnAmt * 0.3,                            y: yBot - dist * fadeStart },
        p3: { x: cx + mirror * (baseTurnAmt * 0.3 - baseFadeAmt * 0.4),      y: yBot - dist },
        distMeters, isHyzerFlip,
      };
    }
    const hyzerFadeAmt = baseFadeAmt * 2.2;
    const hyzerTurnAmt = baseTurnAmt * 0.15;
    return {
      p0: { x: cx,                                                    y: yBot },
      p1: { x: cx - mirror * COL * 0.30,                              y: yBot - dist * bendStart },
      p2: { x: cx + mirror * (hyzerTurnAmt - COL * 0.15),             y: yBot - dist * fadeStart },
      p3: { x: cx + mirror * (hyzerTurnAmt - hyzerFadeAmt),           y: yBot - dist },
      distMeters, isHyzerFlip: false,
    };
  }

  // ANHYZER
  if (rel === 'anhyzer') {
    const stabilityN  = Math.max(0, Math.min(1, (stability - 1.0) / 5));
    const underMult   = 1.0 + Math.max(0, -turnV) * 0.30;
    const anhyzerPeak = COL * 1.4 * (1 + turnExpr * 0.5) * underMult;
    const fadePull    = baseFadeAmt * 2.2 * stabilityN * (0.5 + speedN * 0.5);
    return {
      p0: { x: cx,                                           y: yBot },
      p1: { x: cx,                                           y: yBot - dist * bendStart },
      p2: { x: cx + mirror * anhyzerPeak,                    y: yBot - dist * fadeStart },
      p3: { x: cx + mirror * (anhyzerPeak - fadePull),       y: yBot - dist },
      distMeters, isHyzerFlip: false,
    };
  }
  ...
  return {
    p0: { x: cx,                                          y: yBot },
    p1: { x: cx,                                          y: yBot - dist * bendStart },
    p2: { x: cx + mirror * baseTurnAmt,                   y: yBot - dist * fadeStart },
    p3: { x: cx + mirror * (baseTurnAmt - baseFadeAmt),   y: yBot - dist },
    distMeters, isHyzerFlip: false,
  };
}
      ...
      <div style={{ width:'100%', maxWidth:480 }}>
        ...
        <div style={{ background:BG, borderRadius: isMobile ? 10 : 16, padding: isMobile ? '14px 12px' : '20px 20px 20px', border:'1px solid rgba(255,255,255,0.07)', boxShadow:'0 28px 80px rgba(0,0,0,0.7)', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, position:'relative', zIndex:1 }}>
            <span style={{ fontSize:9, fontWeight:600, letterSpacing:'0.22em', textTransform:'uppercase', color:'rgba(232,228,220,0.30)' }}>Flight Chart</span>
            {selected && (
